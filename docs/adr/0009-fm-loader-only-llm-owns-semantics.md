...
We cut the heuristic semantic stack (region detection, cost-head rollup, formula skeletons, golden snapshots). The Java component is **FM Loader**: ingest workbooks into a cell graph, optionally build a reference graph, export redacted sheets, and accept LLM write-back onto cell rows.
...
5. Export number-redacted sheet views for external models (ADR 0008)
6. Accept analyst review decisions on model proposals via `review_queue`
...
Region geometry, labels, purpose, and scratch marks are proposed by an **external model** from redacted sheets and written back onto cell rows. Amounts never leave the database and are never reconstructed from model output.
