...
/**
 * Tests for {@link DependencyGraphEngine}: Tarjan's SCC cycle detection & workbook calc metadata.
 */
class DependencyGraphEngineTest {
    ...
    void detectsCyclesAndUpdatesWorkbookMetadata() throws Exception {
        try (WorkspaceDatabase db = WorkspaceDatabase.open(tempDir.resolve("graph.db"))) {
            ...
            repo.insertCellReference(idA1, 0, "B1", "local_cell", "Sheet1", ws1, "B1", idB1, null, false, false, 0, 1, false, false, null);
            ...
            engine.processWorkbookGraph(workbookId, parseRunId);
