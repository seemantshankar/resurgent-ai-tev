...
/**
 * Safe formula normalization per parser-strategy-v2 §6:
 * <ul>
    ...
    public static String normalize(String formula) {
        ...
        for (int i = 0; i < work.length(); i++) {
            ...
            if (inDoubleQuote) {
                out.append(c);
                if (c == '"') {
                    // A doubled double quote is an escaped quote inside the string.
            ...
            if (inSingleQuote) {
                out.append(c);
                if (c == '\'') {
                    // A doubled single quote is an escaped quote inside the sheet name.
                    if (i + 1 < work.length() && work.charAt(i + 1) == '\'') {
                        out.append('\'');
                        i++;
                    } else {
